...
for cont,x in enumerate(datos):
    urlscrapcurso=x[0]
    print '%s  %s'%(cont,urlscrapcurso)
    webpagepricipal = urlopen(urlscrapcurso).read() #lectura de la pagina a scrapear 
    soup1 = BeautifulSoup(webpagepricipal)
    tiSoup = soup1.select("div.division-listing")#selecion de la pagina que contiene los titulos de las noticias
    urlcursos=tiSoup[0].select("p.course-listing")
    for contcur,curso in enumerate(urlcursos):
        urlcurso=curso.a.get('href')
        urlscrap=urlcurso
        if contcur<0:
            continue
        
        print '%s.%s  %s'%(cont, contcur,urlscrap)      
