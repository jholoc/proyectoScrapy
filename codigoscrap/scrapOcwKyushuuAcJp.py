...
def identificarOer2(url):
    patron = re.compile("(\.(pdf|mp3|mp4|wmv|zip|rar|tar|gz|htm|html|xls|xlsx|doc|docx|odt|ppt|pptx|XLS|DOCX|PPTX|jpg)$)")
...
for cont,x in enumerate(datos):
    ...
    tiSoup = soup1.select("#sectionLinks > ul > li > div > a")#selecion de la pagina que contiene los titulos de las noticias
    ...
    for i in tiSoup:
        ...
        htmlCurso = soup2.select('#course_main_video')#html del curso
        if htmlCurso==[]:
            htmlCurso = soup2.select('#course_main')#html del curso
        if htmlCurso==[]:
            continue
